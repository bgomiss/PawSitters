            ...
            ZStack {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 32) {
                            SideFilterMenu(selectedTab: $selectedTab, isExpanded: isShowing, location: $location, selectedDateRange: $selectedDateRange, environment: $environment)
                            Button(action: {
                                    ...
                                    userProfileService.fetchUserProfile(uid: user.uid) { result in
                                        ...
                                        case .success(let profile):
                                            DispatchQueue.main.async {
                                                ...
                                                firestoreService.fetchListings(
                                                    for: profile.role,
...
//                                                if let dateRange = selectedDateRange, !location.isEmpty, !environment.isEmpty {
...
struct FilterMenuView_Previews: PreviewProvider {
    static var previews: some View {
        FilterMenuView(isShowing: .constant(true), selectedTab: .constant(0), isExpanded: true, location: "manhattan", environment: "Urban")
    }
}
